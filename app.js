...
const inventors = [
  ...
  let oldestToYoungest = inventors.sort((oldest, youngest) => oldest.year - youngest.year)
  console.log(oldestToYoungest)
  
  
  // 4. Sort the inventors by years lived from shortest to longest-lived
  const inventorsByYearsLived  = inventors.sort((shortest, longest) => (shortest.passed - shortest.year) - (longest.passed - longest.year)
  );
  ...
  const totalYearsLived = inventors.reduce((acc, num) => acc + (num.passed - num.year), 0);
 console.log(totalYearsLived);
  ...
  const first_last = people.map(name => `${name.split(',')[1]} ${name.split(',')[0]}`);
...
//acc = {}
//vehicle = 'car'
//acc[vehicle] = in the empty object does value for the key 'car' exist?
...
//if it does not then assign a value of 1 to acc[vehicle]
  ...
  const whoIs19 = devs.some(x => x.year < 2001)
  console.log(whoIs19)
  
  
  // Array.prototype.every()
  // 9. Check if everyone is 19 or older?
  const whoIs19 = devs.every(x => x.year < 2001)
  console.log(whoIs19)
  ...
  const findComment = comments.find(x => x.id === 823423)
  console.log(findComment)
  
  
  // Array.prototype.findIndex()
  // 11. Find the index of the comment with an id of 123523
  const findCommentIndex = comments.findIndex(x => x.id === 123523)
